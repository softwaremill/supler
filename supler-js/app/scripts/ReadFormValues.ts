class ReadFormValues {
    getValueFrom(element) {
        ...
        for (var i=0; i<children.length; i++) {
            if (ReadFormValues.isElement(children[i])) {
                ...
                if (fieldType) {
                    ...
                    switch (fieldType) {
                        case "string":
                            ReadFormValues.appendFieldValue(result, fieldName, children[i].value, multiple);
                            break;

                        case "integer":
                            ReadFormValues.appendFieldValue(result, fieldName, parseInt(children[i].value), multiple);
                            break;
                    ...
                    Util.foreach(childResult, (field, v) => {
                        ReadFormValues.appendFieldValue(result, field, v, multiple);
                    });
                }
            }
        }
